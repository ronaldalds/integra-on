...
CARGA_INTRACAO_URL = "https://api.desk.ms/ChamadoHistoricos/lista"
...
class Desk:
    def __init__(self) -> None:
        ...
        self.url_carga_interacao = CARGA_INTRACAO_URL
    ...
    def lista_interacao(self, chave: str = ""):
        headers = {"Authorization": self.authentication().token}
        data_json = {
            "Chave":chave,
            "Solicitante":"N",
            "Colunas":{
                ...
                "DataCriacao":"on",
                "Status":"on",
                "FantasiaFornecedor":"on",
                "ChamadoAprovadores":"on",
                "TempoCorridoAcoes":"on",
            }
        }
        response = self._make_request(self.url_carga_interacao, headers, data_json)
        if response.status_code == 200:
            return response.json()
        return {}
